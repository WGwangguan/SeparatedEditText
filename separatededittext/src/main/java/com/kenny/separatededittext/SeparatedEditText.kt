...
class SeparatedEditText @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : AppCompatEditText(context, attrs, defStyleAttr) {
    ...
    private fun drawCursor(canvas: Canvas) {
        if (!isCursorShowing && showCursor && contentText.length < maxLength && hasFocus()) {
            ...
            val startX = spacing * cursorPosition + boxWidth * (cursorPosition - 1) + boxWidth / 2
    ...
    private fun drawRect(canvas: Canvas) {
        for (i in 0 until maxLength) {
            boxRectF[spacing * (i + 1) + boxWidth * i.toFloat(), 0f, spacing * (i + 1) + boxWidth * i + boxWidth.toFloat()] = boxHeight.toFloat()
            if (type == TYPE_SOLID) {
    ...
    override fun onTextChanged(text: CharSequence, start: Int, lengthBefore: Int, lengthAfter: Int) {
        ...
        contentText = text
        invalidate()
        if (textChangedListener != null) if (text.length == maxLength) textChangedListener!!.textCompleted(text) else textChangedListener!!.textChanged(text)
    }
